...
List.prototype.object = function(obj) {
  return join(obj, ", ");
};
...
Cons.prototype.append = function (cons) {
  return immList(new Cons(this.toString().replace(/[\[\]]/g,''), cons));
};
...
function runTests() {
  // ---begin tests for nil---
  runTest(test_nil_instanceof_list);// instanceof
  runTest(test_nil_join);// join
  runTest(test_nil_toString);// toString
  runTest(test_nil_has_no_head);// head
  runTest(test_nil_has_no_tail);// tail
  runTest(test_nil_isEmpty);// isEmpty
  runTest(test_nil_length);// length
  runTest(test_nil_nil_append);// append
  runTest(test_nil_cons_append);
  runTest(test_nil_contains);// contains
  runTest(test_nil_filter);// filter
  runTest(test_nil_map);// map
  // ---end tests for nil---
  // ---begin tests for cons---
  runTest(test_cons_join_single_element);// join
  runTest(test_cons_join_two_elements);
  runTest(test_cons_join_three_elements);
  runTest(test_cons_toString_single_element);// toString
  runTest(test_cons_toString_two_elements);
  runTest(test_cons_toString_three_elements);
  runTest(test_cons_instanceof_list);// instanceof
  runTest(test_cons_head);// head
  runTest(test_cons_empty_tail);// tail
  runTest(test_cons_nonempty_tail);
  runTest(test_cons_isEmpty);
  runTest(test_cons_length_1);// length
  runTest(test_cons_length_2);
  runTest(test_cons_nil_append);// append
  runTest(test_cons_cons_append_1);
  runTest(test_cons_cons_append_2);
  runTest(test_cons_contains_first);// contains
  runTest(test_cons_contains_second);
  runTest(test_cons_contains_nowhere);
  runTest(test_cons_filter_has_element);// filter
